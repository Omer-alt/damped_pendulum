...
class MLP(nn.Module):
    ...
    #         x = nn.relu(x)
    #     x = nn.Dense(self.layers[-1])(x)  # Final layer without activation
    #     return x
    ...
    def __call__(self, x):
        for units in self.layers:
            x = nn.Dense(units)(x)
            x = nn.relu(x)  
        x = nn.Dense(1)(x)  # Output layer for regression (1 output)
        return x
...
def train(model, data, learning_rate, epochs, input_shape, key):
    ...
    train_state = create_train_state(model, key, learning_rate, input_shape)
    ...
    y_train = y_train.reshape(-1, 1)  # Shape (8, 1)
    t_test = t_test.reshape(-1, 1)  # Shape (8, 1)
    y_test = y_test.reshape(-1, 1)  # Shape (8, 1)
    ...
    for epoch in range(epochs):
        ...
        test_metrics = val_step(train_state, val_data)
        
        train_metrics_history.append(train_metrics)
        test_metrics_history.append(test_metrics)

        if epoch % 1000 == 0:
            print(f"Epoch {epoch}, Train Loss: {train_metrics}, Test Loss: {test_metrics['loss']}")

    return train_state,  train_metrics_history, test_metrics_history
